...
[Prompt组装] 将 expression_peak + body_language 注入 Subject 层
       ↓
[视频生成] CogVideoX-3 接收含角色情绪的 Prompt
```
...
参考魔因漫创的 `prompt-builder.ts`，适配 CogVideoX-3 的 95 字限制：
...
├── services/
│   ├── llm_client.py                 # 统一 LLM 客户端(DeepSeek/GLM可切换)
...
复用现有 `prompt_service.py` 的 `AsyncOpenAI` 模式，新建 `llm_client.py`：

```python
class LLMClient:
    """统一 LLM 客户端，支持 DeepSeek 和智谱 GLM 切换"""
...
3. 设置标题、风格(cinematic/anime/ghibli等)、类型(武侠/科幻等)、LLM(DeepSeek/GLM)
...
| 6 | LLM 客户端 | 新建 `services/llm_client.py` | DeepSeek/GLM 可切换 |
...
- CogVideoX-3 对长 Prompt 效果衰减明显
- 与现有 `prompt_service.py` 的字数限制保持一致
- 5层语义组装 + 智能截断确保在限制内最大化信息量

### LLM 可切换的设计理由

- DeepSeek：成本低、速度快、中文理解好
- 智谱 GLM：与 CogVideoX-3 同平台、可能有更好的 Prompt 兼容性
- 两者都兼容 OpenAI API 格式，切换成本极低
