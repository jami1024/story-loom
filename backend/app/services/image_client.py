...
支持 4 种调用方式：
  ...
  - gemini_native: httpx 直接 POST Gemini REST API
"""
import base64
import logging
import re
...
# size → 宽高比映射（gemini_sdk 用原生 aspect_ratio，其余 Gemini 路径 append 到 prompt）
...
class ImageClient:
    ...
    def _load_providers(self, providers: list[dict]):
        for p in providers:
            ...
            if client_type in ("openai_image", "openai_chat"):
                ...
                self._init_genai_client(name, p["api_key"])
            # gemini_native 用 httpx，懒初始化即可

            logger.info(f"图片客户端已初始化: {name} (type={client_type}, model={p['default_model']})")

    def _init_genai_client(self, name: str, api_key: str):
        """初始化 google-genai SDK 客户端"""
        try:
            from google import genai
            self._genai_clients[name] = genai.Client(api_key=api_key)
        except ImportError:
    ...
    async def _generate_gemini_sdk(
        ...
        if not genai_client:
            raise AppException(
                f"Gemini SDK 客户端 '{provider}' 未初始化（google-genai 未安装？）",
                status_code=503,
            )
        ...
        raise AppException("Gemini SDK 返回中未找到图片数据", status_code=502)
    ...
    async def _generate_gemini_native(
        ...
        """通过 httpx 直接 POST Gemini REST API"""
        ...
        if response.status_code != 200:
            logger.error(f"Gemini API 错误: {response.status_code} {response.text[:500]}")
            raise AppException(
                f"Gemini 图片生成失败: HTTP {response.status_code}",
                status_code=502,
            )
        ...
        if not b64_data:
            raise AppException("Gemini Native 返回中未找到图片数据", status_code=502)
    ...
    def _extract_gemini_native_image(data: dict) -> str | None:
        """从 Gemini REST API 响应中提取 base64 图片数据"""
        try:
    ...
    async def close(self):
        ...
        for name, client in self._openai_clients.items():
            await client.close()
            logger.debug(f"OpenAI 客户端 '{name}' 已关闭")
